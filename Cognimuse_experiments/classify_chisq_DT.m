...
%catch
	...
	for j=1:num_clips_in_video
	    clip_features_filename = fullfile(paths.features_root, training.videos{i}, sprintf('%06d.%s', training.start_frame(j), layer));
    ...
    %if ~exist(fullfile('./DT_training/', sprintf('dist_%s.mat', descriptors{i})), 'file')
    	compute_dist('ChiSquared','./DT_training',channels);
    %end
    %if ~exist(fullfile('./DT_training/', sprintf('kernel_%s.mat', descriptors{i})), 'file')	
...
for i=1:length(testing.videos)
    ...
    for j=1:num_clips_in_video
        clip_features_filename = fullfile(paths.features_root, testing.videos{i}, sprintf('%06d.%s', testing.start_frame(j), layer));
        [sz(j,:), clip_feature_vector(j,:)] = read_binary_blob(clip_features_filename);
    end
	%video_features(i,:) = mean(clip_feature_vector,1);
        data = mean(clip_feature_vector,1);
