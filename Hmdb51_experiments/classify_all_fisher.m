...
try
    %load('./DT_training/models_Traj.mat');load('./DT_training/models_HOG.mat'); load('./DT_training/models_HOF.mat'); 
    %load('./DT_training/models_MBHx.mat');load('./DT_training/models_MBHy.mat'); load('./DT_training/models_MBH.mat');
    %load('./DT_training/models_c3d.mat'); load('./DT_training/models_tsn_rgb.mat'); load('./DT_training/models_tsn_flow.mat');
    load('./DT_training/models_*.mat');
catch
    %% load C3D features
    ...
    for i=1:length(training.videos)
        ...
        load(['/Data/hmdb51_DT/' training.videos{i} '.mat']);
        switch encoding_type
            case 'BoVW'
	...
	encoded_features = parload(fullfile('./DT_training', sprintf('encoded_features_%s.mat', descriptors{id})));
    	...
    	parfor c = 1:length(classes)
        	...
        	w_neg = length(training.video_gt)/(2*sum(~oneVSall));
       		parameters = sprintf('-t 0 -q -c %d -w1 %f -w0 %f', svm_cost, w_pos, w_neg);
%         	parameters = sprintf('-t 0 -b 1 -q -c %d', svm_cost);
        	models{c} = svmtrain(double(oneVSall), double(encoded_features), parameters);
%    		end
...
for i=1:length(testing.videos)
        ...
        load(['/Data/hmdb51_DT/' testing.videos{i} '.mat']);
        switch encoding_type
            case 'BoVW'
                encoded_features = encode_multichannel_bof(f, channels, 0);
            case 'fisher'
...
for id=1:length(descriptors)
	...
	for m=1:length(models)
    		...
    		[~,~,p] = svmpredict(zeros(size(encoded_features,1),1), double(encoded_features), models{m} ,'-b 1 -q');
